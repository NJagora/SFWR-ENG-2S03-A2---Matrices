...
public class MatrixArrayCR {
	...
	public MatrixArrayCR (int n, long[] in_array) throws WrongLength{
	...
	public long[][] transpose(long[][] in_mat){
		long[][] out_mat = new long[in_mat.length][in_mat.length]; //declares a 2D array of the same size as in_mat
		for (int i = 0; i < in_mat.length; i++){
			for (int j = 0; j < in_mat.length; j++){
				//for every iteration of this loop, the value of out_mat[i][j]
				//is changed to in_mat's transpose's value at the same location
